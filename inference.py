...
def inference(model, output, csv, imgpath, valid):
    ...
    if valid:
        ground_truth_csv = pd.read_csv(os.path.join("single_instance_dataset_wradius.csv"))
    ...
    valid_generator = utils.DataGeneratorMobileNetKeras(batch_size=1, img_path=img_path, labels=labels,
        ...
        for i in np.arange(0, prediction.shape[0]):
            ...
            if valid:
                ...
                ground_truth = (utils.read_image_grayscale(
                    os.path.join(
                        mask_path, 'mask_'+test_images[i][:-3]+'png')))
                ...
                if centers is not None:
                    for component_label in range(num_labels):
                        ...
                        # relative_area
                        data['component_relative_area'] = component_area / prediction_area
