...
def mobilenet_8s(train_encoder=True, final_layer_activation="sigmoid", prep=True):
    ...
    if prep == True:
        net.load_weights(
            os.path.join(".", "mn_classification_weights.h5"), by_name=True
        )
    else:
        net.load_weights(
            os.path.join(".", "test_preprocessing_weights.h5"), by_name=True
        )
    ...
    for layer in net.layers:
        ...
        deconv2 = Conv2DTranspose(
            filters=1, kernel_size=4, strides=2, padding="same", use_bias=False
        )(predict)
