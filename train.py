...
@click.option(
    "--savemodel", default=True, is_flag=True, help="Save to trained model weights"
)
@click.option(
    "--csv", default="./train.csv", help="Training set csv with (image, mask) tuples"
)
...
def train(epochs, lr, bs, savemodel, csv, imgpath):
    ...
    if not os.path.exists(os.path.join("mn_classification_weights.h5")):
        ...
        gdown.cached_download(
            url, output, md5=WEIGHTS_CHECKSUM, postprocess=gdown.extractall
        )
    ...
    for arch in [8, 16, 32]:
        ...
        if arch == 8:
            model = mobilenet_8s(
                train_encoder=True, final_layer_activation="sigmoid", prep=True
            )
        if arch == 16:
            model = mobilenet_16s(
                train_encoder=True, final_layer_activation="sigmoid", prep=True
        ...
        train_history = model.fit_generator(
            generator=train_generator, use_multiprocessing=True, epochs=epochs
        )
